...
#    txn = transactions.where("type in ('PaymentAuthorize', 'PaymentCharge')").order('type, created_at DESC').first
  ...
  def charge(order, amount, comment)
    txn = transactions.where(:type => 'PaymentAuthorize').where("amount >= ?", amount.to_i).where("created_at > ?", Time.now-7.days).where('active').order("order_id != #{order.id}, amount DESC").first
